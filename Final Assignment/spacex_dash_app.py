...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        ...
        names='Launch Site', 
        title='Total Success Launches On All Sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == site_dropdown]
        fig = px.pie(filtered_df, names='class', title='Total Success Launches By Sites')
        return fig
...
def get_scatter_plot(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        ...
        site_data = spacex_df[spacex_df['Launch Site'] == site_dropdown]
        filtered_df = site_data[(site_data['Payload Mass (kg)'] >= payload_slider[0]) & (site_data['Payload Mass (kg)'] <= payload_slider[1])]
        fig = px.scatter(data_frame=filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig
